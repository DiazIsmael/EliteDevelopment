...
onMounted(loadAccountStatus)
</script>

<template>
  <div class="mb-4.5">
    <label class="mb-2.5 block text-black dark:text-white"> Account Status 
    ...
    <div class="relative z-20 bg-transparent dark:bg-form-input">
      ...
      >
        <option value="" disabled selected>Set account status</option>
        <option v-for="(status) in accountStatus" :key="status.StatusCode" :value="status.StatusCode">{{ status.Status }}</option>
      </select>
