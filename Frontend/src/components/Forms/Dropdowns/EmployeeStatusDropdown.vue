...
import { getEmployeeStatus } from '@/api/api.js'
...
onMounted(loadEmployeeStatus)
</script>

<template>
  <div class="mb-4.5">
    <label class="mb-2.5 block text-black dark:text-white"> Employee Status 
    ...
    <div class="relative z-20 bg-transparent dark:bg-form-input">
      ...
      >
        <option value="" disabled selected>Set employee status</option>
        <option v-for="(status) in employeeStatus" :key="status.StatusCode" :value="status.StatusCode">{{ status.Status }}</option>
      </select>
