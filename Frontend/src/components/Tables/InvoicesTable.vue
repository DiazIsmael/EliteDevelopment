...
import { getInvoices } from '@/api/api.js';
...
// Method to fetch/GET invoice table data from the backend API
const loadInvoices = async () => {
  try {
    ...
    console.error("There was an error fetching the invoices:", err)
    isLoading.value = false
  }
}
...
//Method to redirect to respective invoice details page
const viewInvoiceDetails = (invoiceID) => {
  router.push({ name: 'InvoiceDetails', params: { 'invoiceID': invoiceID } })
}
</script>
        ...
        <tbody>
          <tr v-for="(invoice, index) in invoices" :key="index">
            <td class="py-5 px-4 pl-9 xl:pl-11">
              ...
              <p class="text-sm text-meta-3">Billed: ${{ invoice.Total }}</p>
            </td>
            ...
            <td class="text-center py-5 px-4">
              ...
              <p v-if="invoice.StatusCode == 2" 
                class="inline-flex rounded-full bg-opacity-10 py-1 px-4 text-sm font-medium"
                :class="{'bg-danger text-danger': invoice.StatusCode == 2}"> Unpaid
              </p>
              <p v-if="invoice.StatusCode == 1" 
                class="inline-flex rounded-full bg-opacity-10 py-1 px-4 text-sm font-medium"
            ...
            <td class="items-center py-5 px-4">
              <div class="flex justify-center space-x-3.5">
                <button @click="viewInvoiceDetails(invoice.InvoiceID)" class="hover:text-primary">
                  <svg
                    class="fill-current"
                    width="18"
                    height="18"
