getIncidents<script setup lang="ts">
import { ref, onMounted } from 'vue'
import { getServiceIncidents } from '@/api/api.js';

const serviceincidents = ref([
  {
    ServiceIncidentID: 0,
    WorkOrderID: 0,
    EmployeeID: 0,
...
const loadServiceIncidents = async () => {
  try {
    const response = await getServiceIncidents()
    serviceincidents.value = response
    isLoading.value = false
  } catch (err) {
    console.error("There was an error fetching the service incidents:", err)
    isLoading.value = false
  }
}

onMounted(loadServiceIncidents)
</script>

<template>
  <div class="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
    <h4 class="mb-6 text-xl font-semibold text-black dark:text-white">Service Incidents</h4>
    <div class="flex flex-col">
      <div class="grid grid-cols-4 rounded-sm bg-gray-2 dark:bg-meta-4 sm:grid-cols-6">
        <div class="p-2.5 text-center xl:p-5">
          <h5 class="text-sm font-medium uppercase xsm:text-base">WorkOrder ID</h5>
        </div>
      ...
      <div
        v-for="(incident,key) in serviceincidents"
        :key="incident.ServiceIncidentID"
        :class="`grid grid-cols-4 sm:grid-cols-6 ${key === serviceincidents.length - 1 ? '' : 'border-b border-stroke dark:border-strokedark'}`">
        
        <div class="flex items-center justify-center p-2.5 xl:p-5">
          <p class="font-bold text-black dark:text-white sm:block">{{ incident.WorkOrderID }}</p>   
        </div>
